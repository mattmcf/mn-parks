...
const TYPE_ICONS: Record<ParkType, string> = {
  ...
  wilderness: `<svg ${ICON_ATTRS}><path d="M12 21v-5"/><path d="m7 14 5-8 5 8H7Z"/><path d="m8.5 17 3.5-5 3.5 5H8.5Z"/></svg>`,
...
export function createParkMarkerElement(park: Park, onSelect: (park: Park) => void) {
  ...
  button.setAttribute("aria-label", `${park.name}, ${TYPE_LABELS[park.park_type]}`)
